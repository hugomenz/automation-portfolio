...
export function buildFlagshipN8nExport(workflow) {
  ...
  nodes.push(
    ...
    node(workflow, 'agent-two', profile.agentTwo[0], '@n8n/n8n-nodes-langchain.agent', 2.2, [1250, -120], { promptType: 'define', text: '={{ $json.agentTwoPrompt }}', options: { systemMessage: 'You are an independent industrial risk challenger. Preserve source boundaries, challenge unsupported claims, and never release a payment, approve a price, diagnose a machine, or write to a real system. Return only valid JSON.', maxIterations: 4, returnIntermediateSteps: true, passthroughBinaryImages: false } }, { onError: 'continueErrorOutput' }),
    ...
    code(workflow, 'case-record', 'Prepare Supabase Case Record', [2040, -120], `return items.map(item => ({ json: { correlation_id: item.json.correlationId, workflow_code: item.json.workflowCode, title: ${JSON.stringify(profile.name + ' - ' + scenario.label)}, source_type: item.json.sourceType, source_asset_url: item.json.sourceAssetUrl, status: item.json.status, priority: ${JSON.stringify(profile.priority)}, current_department: ${JSON.stringify(profile.currentDepartment)}, extracted_data: { visual: item.json.visualEvidence, evidence_agent: item.json.agentResult, challenger: item.json.challengeResult, prepared_output: item.json.preparedOutput }, exception_data: { exceptions: item.json.exceptions, signals: item.json.signals }, human_decision: item.json.humanDecision } }));`),
    ...
    code(workflow, 'agent-record', 'Build Agent Run Audit', [2570, -120], `const run = $('Deterministic Decision Guardrails').first().json; return [{ json: { case_id: $json.id, correlation_id: run.correlationId, agent_name: ${JSON.stringify(profile.agentOne[0] + ' + ' + profile.agentTwo[0])}, model_provider: 'Groq', model_id: 'qwen/qwen3.6-27b + llama-3.3-70b-versatile', task: ${JSON.stringify(profile.name)}, status: run.status, confidence: run.confidence, input_refs: { source_asset_url: run.sourceAssetUrl, source_type: run.sourceType }, output: { evidence_agent: run.agentResult, challenger: run.challengeResult }, error: run.technicalFailure || null, latency_ms: null } }];`),
    ...
    code(workflow, 'event-record', 'Build Department Handoff Event', [3100, -120], `return [{ json: { case_id: $json.case_id, event_type: 'HUMAN_REVIEW_REQUESTED', department: ${JSON.stringify(profile.currentDepartment)}, actor_type: 'workflow', actor_label: ${JSON.stringify(profile.name)}, payload: { status: $json.status, agent_name: $json.agent_name, external_write_performed: false } } }];`),
    ...
    code(workflow, 'approval-record', 'Prepare Human Approval Tasks', [3620, -120], `const caseId = $json.case_id; return ${JSON.stringify(profile.approvalDepartments)}.map(department => ({ json: { case_id: caseId, department, decision: 'pending', decided_by: null, rationale: 'Synthetic demo - explicit human approval required', version: 1 } }));`),
  ...
  profile.toolDomains.forEach(([toolName, domain, description], index) => {
    nodes.push(node(workflow, `tool-${index}`, toolName, 'n8n-nodes-base.supabaseTool', 1, [470 + index * 210, 300], { toolDescription: description, resource: 'row', operation: 'getAll', tableId: 'industrial_lab_master_data', returnAll: true, filterType: 'string', filterString: `domain=eq.${domain}` }));
  });
