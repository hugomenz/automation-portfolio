...
export function buildFlagshipN8nExport(workflow) {
  ...
  nodes.push(
    ...
    code(workflow, 'case-record', 'Prepare Supabase Case Record', [2040, -120], `const safe = value => value === undefined ? null : JSON.parse(JSON.stringify(value)); const candidates = items.map(item => item.json || {}); const source = candidates.find(item => !item.technicalFailure && item.challengeResult && !item.challengeResult.fallback) || candidates.find(item => !item.technicalFailure) || candidates[0] || {}; return [{ json: { id: ${JSON.stringify(uuidFrom(`industrial-lab-case-${workflow.id}`))}, correlation_id: String(source.correlationId || source.sourceInput?.eventId || 'MISSING'), workflow_code: String(source.workflowCode || ${JSON.stringify(workflow.code)}), title: ${JSON.stringify(profile.name + ' - ' + scenario.label)}, source_type: String(source.sourceType || 'synthetic_document'), source_asset_url: String(source.sourceAssetUrl || ''), status: String(source.status || 'HUMAN_REVIEW_REQUIRED'), priority: ${JSON.stringify(profile.priority)}, current_department: ${JSON.stringify(profile.currentDepartment)}, extracted_data: { visual: safe(source.visualEvidence), evidence_agent: safe(source.agentResult), challenger: safe(source.challengeResult), prepared_output: safe(source.preparedOutput) }, exception_data: { exceptions: safe(Array.isArray(source.exceptions) ? source.exceptions : []), signals: safe(Array.isArray(source.signals) ? source.signals : []) }, human_decision: safe(source.humanDecision || { required: true, state: 'pending', owner: ${JSON.stringify(profile.approvalDepartments.join(' + '))} }) } }];`),
    node(workflow, 'create-case', 'Supabase - Atomic Case + Idempotency Claim', 'n8n-nodes-base.supabase', 1, [2310, -120], { resource: 'row', operation: 'create', tableId: 'industrial_lab_cases', dataToSend: 'autoMapInputData', inputsToIgnore: '' }, { onError: 'continueErrorOutput', alwaysOutputData: true }),
    code(workflow, 'agent-record', 'Build Agent Run Audit', [2570, -120], `const run = $('Deterministic Decision Guardrails').first().json; const caseId = $('Prepare Supabase Case Record').first().json.id; const agentStatus = run.technicalFailure ? 'failed' : 'completed'; return [{ json: { case_id: caseId, correlation_id: String(run.correlationId || run.sourceInput?.eventId || 'MISSING'), agent_name: ${JSON.stringify(profile.agentOne[0] + ' + ' + profile.agentTwo[0])}, model_provider: 'Groq', model_id: 'qwen/qwen3.6-27b + llama-3.3-70b-versatile', task: ${JSON.stringify(profile.name)}, status: agentStatus, confidence: Number(run.confidence || 0), input_refs: { source_asset_url: run.sourceAssetUrl || '', source_type: run.sourceType || 'synthetic_document' }, output: { business_status: String(run.status || 'HUMAN_REVIEW_REQUIRED'), evidence_agent: run.agentResult || null, challenger: run.challengeResult || null }, error: run.technicalFailure || null, latency_ms: null } }];`),
    node(workflow, 'create-agent-run', 'Supabase - Persist Agent Audit', 'n8n-nodes-base.supabase', 1, [2840, -120], { resource: 'row', operation: 'create', tableId: 'industrial_lab_agent_runs', dataToSend: 'autoMapInputData', inputsToIgnore: '' }, { onError: 'continueErrorOutput', alwaysOutputData: true }),
    code(workflow, 'event-record', 'Build Department Handoff Event', [3100, -120], `const run = $('Build Agent Run Audit').first().json; return [{ json: { case_id: run.case_id, event_type: 'HUMAN_REVIEW_REQUESTED', department: ${JSON.stringify(profile.currentDepartment)}, actor_type: 'system', actor_label: ${JSON.stringify(profile.name)}, payload: { agent_status: run.status, business_status: run.output?.business_status || 'HUMAN_REVIEW_REQUIRED', agent_name: run.agent_name, external_write_performed: false } } }];`),
    node(workflow, 'create-event', 'Supabase - Append Audit Event', 'n8n-nodes-base.supabase', 1, [3360, -120], { resource: 'row', operation: 'create', tableId: 'industrial_lab_events', dataToSend: 'autoMapInputData', inputsToIgnore: '' }, { onError: 'continueErrorOutput', alwaysOutputData: true }),
    ...
    node(workflow, 'create-approval', 'Supabase - Create Approval Queue', 'n8n-nodes-base.supabase', 1, [3880, -120], { resource: 'row', operation: 'create', tableId: 'industrial_lab_approvals', dataToSend: 'autoMapInputData', inputsToIgnore: '' }, { onError: 'continueErrorOutput', alwaysOutputData: true }),
    code(workflow, 'human-gate', 'HUMAN GATE - Review Required', [4140, -120], `const run = $('Deterministic Decision Guardrails').first().json; return [{ json: { correlationId: run.correlationId, status: run.status, approvalTasks: ${JSON.stringify(profile.approvalDepartments)}.map(department => ({ department, decision: 'pending' })), preparedOutput: run.preparedOutput, externalWritePerformed: false, terminalState: 'WAITING_FOR_HUMAN_DECISION', evidence: { source: run.sourceAssetUrl, groq: true, supabase: true } } }];`),
