...
From the repository root run `npm run check`. Open `/lab/spare-parts-inquiry/` from the built site and run all three cases. The matching sanitized n8n export is `n8n/workflows/spare-parts-inquiry.workflow.json`; it is disabled and executes all three synthetic cases from a Manual Trigger.
