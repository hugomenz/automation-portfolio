...
From the repository root run `npm run check`. Open `/lab/rfq-prequalification/` from the built site and run all three cases. The matching sanitized n8n export is `n8n/workflows/rfq-prequalification.workflow.json`; it is disabled and executes all three synthetic cases from a Manual Trigger.
